...
{
    // To complete
    uint32_t target_ticks = get_ticks_from_reset() + ticks_to_wait;
    
    while(get_ticks_from_reset() < target_ticks);
    // Wait in an active loop for the number of ticks
...
void wait_until( uint64_t target_ticks )
{
    ...
    // Wait in an active loop until the current ticks reaches
